...
import { runMacro, listMacros } from './commands/runMacroCommands';
...
export class VbaSyncExtension {
    ...
    async activate(context: vscode.ExtensionContext) {
        console.log('AutoSyncVBA: 开始激活插件...');
        try {
            // 初始化状态栏
            console.log('AutoSyncVBA: 初始化状态栏');
            this.statusBar = new StatusBar(null as any);
            this.statusBar.updateStatus();

            // 环境检测
            console.log('AutoSyncVBA: 开始环境检测');
            const envResult = await EnvironmentSetup.ensureEnvironment();
            console.log('AutoSyncVBA: 环境检测结果:', envResult);

            if (!envResult.pythonReady) {
                console.log('AutoSyncVBA: Python 环境未就绪');
                vscode.window.showWarningMessage('Python 环境未就绪，AutoSyncVBA 功能将受限');
                // 仍然注册命令，只是功能受限
                console.log('AutoSyncVBA: 注册命令');
                this.registerCommands(context);
                return;
            }

            // 初始化组件
            console.log('AutoSyncVBA: 初始化 PythonClient');
            this.pythonClient = new PythonClient();
            
            console.log('AutoSyncVBA: 初始化 ConnectionManager');
            this.connectionManager = new ConnectionManager(this.pythonClient);
            
            console.log('AutoSyncVBA: 初始化 ConfigManager');
            this.configManager = new ConfigManager();
            
            console.log('AutoSyncVBA: 初始化 StateManager');
            this.stateManager = new StateManager();
            
            console.log('AutoSyncVBA: 初始化 NotificationManager');
            this.notificationManager = new NotificationManager();
            
            console.log('AutoSyncVBA: 初始化 SyncEngine');
            ...
            console.log('AutoSyncVBA: 初始化 FileWatcher');
            this.fileWatcher = new FileWatcher(this.syncEngine);
            
            // 更新状态栏
            console.log('AutoSyncVBA: 更新状态栏');
            ...
            // 清理无效连接
            console.log('AutoSyncVBA: 清理无效连接');
            try {
                await this.connectionManager?.cleanupConnections();
                console.log('AutoSyncVBA: 连接清理完成');
            } catch (error) {
                console.error('AutoSyncVBA: 连接清理失败:', error);
            }

            // 检测并连接已打开的文档
            console.log('AutoSyncVBA: 检测并连接已打开的文档');
            try {
                await this.connectionManager?.detectAndConnectToOpenDocuments();
                console.log('AutoSyncVBA: 已成功连接已打开的文档');
            } catch (error) {
                console.error('AutoSyncVBA: 检测并连接已打开的文档失败:', error);
            }

            console.log('AutoSyncVBA: 激活完成');
            vscode.window.showInformationMessage('AutoSyncVBA 已激活');
        } catch (error) {
            console.error('AutoSyncVBA: 激活失败:', error);
            vscode.window.showErrorMessage(`AutoSyncVBA 激活失败: ${error}`);
        }
    }
    ...
    private registerCommands(context: vscode.ExtensionContext) {
        const commands = [
            ...
            vscode.commands.registerCommand('vba-sync.runMacro', () => 
                runMacro(this.syncEngine!)
            ),
            vscode.commands.registerCommand('vba-sync.listMacros', () => 
                listMacros(this.syncEngine!)
            ),
        ];
